...
from .utils.logging import log_function_call, log_error, log_api_request, RequestLogger
import logging
...
def start_investing(request):
    with RequestLogger(request):
        try:
            ...
            log_error(e, {'view': 'start_investing'})
            return HttpResponseServerError("Internal Server Error")
...
def register_startup(request):
    with RequestLogger(request):
        try:
            ...
            log_error(e, {'view': 'register_startup'})
            return HttpResponseServerError("Internal Server Error")
...
def about(request):
    with RequestLogger(request):
        try:
            ...
            log_error(e, {'view': 'about'})
            return HttpResponseServerError("Internal Server Error")

@log_function_call
def custom_404(request, exception=None):
    """Custom 404 handler"""
    log_error(exception, {'path': request.path, 'status': 404})
    return render(request, 'errors/404.html', status=404)

@log_function_call
def custom_500(request):
    """Custom 500 handler"""
    log_error("Internal Server Error", {'path': request.path, 'status': 500})
    return render(request, 'errors/500.html', status=500)

@log_function_call
def custom_403(request, exception=None):
    """Custom 403 handler"""
    log_error(exception, {'path': request.path, 'status': 403})
    return render(request, 'errors/403.html', status=403)

@log_function_call
def handle_template_error(request, template_name, context=None, status=500):
    """Generic template error handler"""
    try:
        ...
        log_error(e, {'template': template_name, 'context': context})
        return HttpResponseServerError("Internal Server Error")